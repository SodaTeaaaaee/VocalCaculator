...
3. 后续每完成一个阶段，若无主路径级硬阻塞，自动进入下一阶段
4. 不得跳阶段宣称“项目已完成”
...
默认用户长时间离线。
执行模型必须尽量把整个项目一次性推进到完成态，而不是在阶段中途频繁暂停。

因此：

1. 局部阻塞不等于全局阻塞；Android、签名、模拟器、外部音乐资产、第三方授权等问题，默认只阻塞对应子路径
2. 某一阶段内若有一项检查失败，先修复；若暂时无法修复，则记录并继续推进其他不依赖该项的工作
3. 不得因为子 agent 不可用、同步文件缺失或联网研究短暂失败就中止整个项目；主 agent 必须补做等价工作并继续
4. 只有桌面主路径、核心构建能力或仓库关键输入本身完全不可用时，才允许停止整个项目
